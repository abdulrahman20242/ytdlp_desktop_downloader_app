...
> **⚠️ Superseded:** This document describes the design intent (May 2026) and contains
> outdated claims (Python API engine, 4-tab settings, nightly channel, `build.spec`,
> stale config keys). The current spec is **`PRD_YTDownloader_v3.md`** (v3.0, Sep 2026).
> Kept for historical reference only; the implementation diff is in `PRD_DIFF.md`.
...
| JS Support | yt-dlp-ejs + Node.js | ≥0.8.0 / Node 20 LTS | `js_runtimes: {"node": {}}` + `pip install yt-dlp-ejs` |
...
**خيار 2 — استيراد من المتصفح مباشرة:**
```python
...
```python
# لا تمرر أي خيارات cookies
# يُستخدم عموماً لمعظم الفيديوهات العامة
```

> **ملاحظة:** الافتراضي هو `"source": "none"` في `config.json` لتجنب خطأ `Could not copy Chrome cookie database` عند فتح المتصفح. المستخدم يفعّلها من الإعدادات عند الحاجة.
الواجهة تعرض Dropdown في `⚙ إعدادات` → متقدم لاختيار المصدر (none / browser / file).
...
```python
# الأولوية: mp4+m4a أولاً (توافق أعلى) ← ثم أي video+audio ← ثم combined fallback
FORMAT_MAP = {
...
بدون `format_sort`، yt-dlp يختار `av01 > vp9 > avc1`. هذا يؤدي لاختيار `398+251` (av01+opus) بدلاً من `136+140` (avc1+m4a).

```python
opts['format_sort'] = ['vcodec:h264,vp9,av01', 'res', 'br']
# يفضّل H.264 ← ثم VP9 ← ثم AV01 للتوافق الأوسع
```

### 9.4 مقاومة Throttling

YouTube يخنق السرعة عند اكتشافه تحميلاً. الحل:

```python
opts['throttledratelimit'] = 102400  # 100 KB/s
# لو السرعة نزلت تحت 100KB/s → yt-dlp يعيد الـ extraction تلقائياً
```

### 9.5 JavaScript Runtime (yt-dlp-ejs)

لحل JS challenges من YouTube عبر Node.js:
...
| `Could not copy Chrome cookie` | كروم مفتوح أو قاعدة بيانات cookies مقفلة | "فشل استخراج cookies — أغلق المتصفح أو استخدم ملف cookies" | تعطيل cookies أو إغلاق المتصفح |
...
**السبب:**
- yt-dlp افتراضياً يفضّل `av01 > vp9 > avc1` لأنه "أحدث"
- AV01 له توافق محدود مع المشغلات القديمة و bitrate أعلى لنفس الجودة
...
**القرار:** استخدام نافذة `CTk` واحدة فقط للنافذة الرئيسية، والمكونات (StartupCheck, MainWindow) تكون `CTkFrame`.

**السبب:**
- إنشاء `CTk` متعددة (واحدة لفحص التبعيات وأخرى للنافذة الرئيسية) يسبب تعارض `after` callbacks الداخلية لـ customtkinter عند إتلاف النافذة الأولى
...
**البديل المرفوض:** إنشاء `CTk` منفصلة لكل نافذة + `withdraw()` بدلاً من `destroy()` → يبقي `after` callbacks نشطة لكن تستهلك موارد.
...
| التاريخ | التغيير |
|---|---|
| مايو 2026 | **v2.0** — الإصدار الأولي من PRD |
| مايو 2026 | **تعديلات التنفيذ:** `format_sort` لتفضيل H.264 (avc1)، `throttledratelimit` لمقاومة التباطؤ، تفعيل `yt-dlp-ejs` عبر `js_runtimes` و `extractor_args`، تغيير هيكل النوافذ إلى Single-Root Architecture (CTkFrame)، تغيير مصدر cookies الافتراضي إلى `none`، إضافة `bin/` إلى PATH في `app.py` |
